...
This guide explains how to integrate your Ketamine Dashboard with your Roblox game.
...
-- CONFIGURATION
-- Replace with your actual deployed domain (e.g., https://ketamine-dashboard.vercel.app)
-- DO NOT include a trailing slash
local API_URL = "http://localhost:3000" 
local API_KEY = "" -- If you add API key authentication later

function Ketamine.CheckUser(player)
	...
	if success then
		...
		warn("Ketamine API Error: " .. tostring(response))
		return nil
	end
end
...
```lua
local Ketamine = require(game.ServerScriptService.Ketamine)

game.Players.PlayerAdded:Connect(function(player)
	print("Checking user: " .. player.Name)
	
	local userData = Ketamine.CheckUser(player)
...
## Example: "Ketamine Loaded" UI Notification

This script will automatically check every player who joins. If they have a Ketamine account, it will display a cool "Ketamine Loaded" notification on their screen.

**Instructions:**
1. Create a **Script** in `ServerScriptService`.
2. Paste the code below.
3. Ensure you have the `Ketamine` ModuleScript set up as described above.
...
-- Require the Ketamine Module (assuming it's in ServerScriptService)
local Ketamine = require(game.ServerScriptService.Ketamine)
...
local function createNotification(player)
	...
	screenGui.Name = "KetamineNotification"
	...
	textLabel.Text = "Ketamine Loaded"

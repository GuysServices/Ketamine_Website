...
export async function sendLicenseKeyEmail(email: string, key: string) {
  try {
    ...
    const { data, error } = await resend.emails.send({
      from: 'Ketamine <onboarding@resend.dev>', // Default Resend test domain, user should update if they have a domain
      to: [email],
      subject: 'Your Ketamine License Key',
      html: `
        <div style="font-family: Arial, sans-serif; bg-color: #0f172a; color: #fff; padding: 20px;">
          <h1 style="color: #3b82f6;">Ketamine</h1>
          ...
          <br/>
          <p style="color: #64748b; font-size: 12px;">The Ketamine Team</p>
        </div>
      `,
    });
